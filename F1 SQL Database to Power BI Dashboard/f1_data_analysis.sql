...
FROM 'C:\repos\f1\driver_standings.csv'
WITH (
    FIELDTERMINATOR = ',',
    ROWTERMINATOR = '\n',
    FIRSTROW = 2
...
BULK INSERT races
FROM 'C:\repos\f1\constructors.csv'
WITH (
    FIELDTERMINATOR = ',',
    ROWTERMINATOR = '\n',
    FIRSTROW = 2
...
ALTER TABLE drivers
ADD fullname AS CONCAT (forename, ' ', surname);

---------------------------------

-- Data Analysis Questions:

-- 1. Who has a higher average points per grand prix between driver1 and driver2?
-- 2. Who has the lowest average lap time at each circuit? Lowest average lap time overall?
-- 3. Who has the highest percent of podium finishes? What is the YoY% difference in number of podium finishes for each driver?
-- 4. Who has the best average rank of fastest lap times? At which laps do they have the highest tendency to run their fastest lap?
-- 5. Compare stats in first 3 years of their careers (points, podium finishes, fastest laps)
